...
| 3 | **Coverage del backend ≥ 60%** | ✅ **98,05%** con los 34 tests pasando (26-09-2026), muy por encima del `--cov-fail-under=60` que exige el CI. Ese número incluye los archivos de test: ver deuda #8 |
...
8. **El 98% incluye los propios archivos de test**, que siempre dan 100% y son casi la mitad de las líneas contadas. Se afina con un `.coveragerc` que tenga `omit = */tests/*`. Ojo al hacerlo: si además no se agrega `concurrency = thread,greenlet`, el número se desploma sin motivo real, porque SQLAlchemy async ejecuta el trabajo de BD dentro de greenlets y `coverage` no traza ahí. Las dos líneas van juntas o el resultado engaña.

   **Cómo medirlo bien:** el porcentaje solo es comparable si los 34 tests pasan. Sin red, los 2 tests que golpean la API de divisas fallan (deuda #3) y arrastran la cifra varios puntos hacia abajo, lo que hace parecer que la cobertura bajó cuando lo que falta es internet.
